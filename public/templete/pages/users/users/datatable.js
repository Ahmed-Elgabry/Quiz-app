...
var KTDatatableRecordSelectionDemo = function() {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: BASE_URL + '/u/users', //BASE_URL >> custom var
                    method: 'GET',
                },
                map: function (data) {
                    return data.data;
        ...
        columns: [{
            ...
            template: function(row) {
	            return '\
                    ...
                    <button class="btn btn-sm btn-clean btn-icon btn-icon-sm open-modal-upgrade" data-value="'+row.id+'" title="'+Lang.get('front-end.users.scripts.button.upgrade')+'">\
                        <i class="flaticon2-check-mark"></i>\
                    </button>\
                    <button class="btn btn-sm btn-clean btn-icon btn-icon-sm open-modal-block" data-value="'+row.id+'" title="'+Lang.get('front-end.users.scripts.button.block')+'">\
                        <i class="fa fa-ban"></i>\
                    </button>\
                ';
            },
    ...
    var localSelectorDemo = function() {
        ...
        $("#btn-ids").click(function (e) {
            ...
            Swal.fire({
                title: Lang.get('front-end.users.scripts.sweetalert.blockall'),
                type: 'warning',
                        ...
                        success: function(data) {
                            ...
                            if (data.msg == 'success') {
                                Swal.fire(
                                    ...
                                    Lang.get('front-end.users.scripts.sweetalert.user.allblocked'),
                                    "success"
                                    ).then((result) => {
                                    // Reload the Page
                                    location.reload();
