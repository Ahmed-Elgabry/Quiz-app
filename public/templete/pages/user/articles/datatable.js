...
var KTDatatableRecordSelectionDemo = function() {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: BASE_URL + '/u/userarticles', //BASE_URL >> custom var
                    method: 'GET',
                },
                map: function (data) {
                    return data.data;
        ...
        columns: [{
	        ...
	        autoHide: false,
            template: function(row) {
                /* if(row.status == 'Approved' || row.status == 'تمت الموافقة' || row.status == 'Pending' || row.status == 'قيد الإنتظار'){
                    var edit_btn = '<button class="btn btn-sm btn-clean btn-icon btn-icon-sm open-modal-edit" data-value="'+row.id+'" title="'+Lang.get('front-end.datatable.edit')+'"><i class="la la-edit"></i></button>';
                }else{
                    var edit_btn = '';
                }

                if(row.status == 'Rejected' || row.status == 'مرفوض' || row.status == 'Pending' || row.status == 'قيد الإنتظار'){
                    var delete_btn = '<button class="btn btn-sm btn-clean btn-icon btn-icon-sm open-modal-delete" data-value="'+row.id+'" title="'+Lang.get('front-end.datatable.delete')+'"><i class="la la-remove"></i></button>';
                }else{
                    var delete_btn = '';
                }
	            return '\
    ...
    var localSelectorDemo = function() {
        ...
        $("#btn-ids").click(function (e) {
            ...
            for (var i = 0; i < ids.length; i++) {
             ...
             var url = BASE_URL + "/u/userarticles/deleteall";
            Swal.fire({
